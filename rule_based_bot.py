...
class SupportBot:
    ...
    def __init__(self):
        self.matching_phrases = {'how_to_pay_bill': [r'.*how.*pay bills.*', r'.*how.*pay my bill.*'], r'pay_bill': [
            r'.*want.*pay.*my.*bill.*account.*number.*is (\d+)', r'.*need.*pay.*my.*bill.*account.*number.*is (\d+)']}
    ...
    def how_to_pay_bill_intent(self):
        return input(f"You can pay your bill a couple of ways. 1) online at bills.chris.com or 2) you can pay  right now with me. Can I help you with anything else, {self.name}? ")
